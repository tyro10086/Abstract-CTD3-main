...
def eval_trad(env, agent, env_config, algo_config, mdl):
    ...
    for t_step in tqdm(range(max_timesteps)):
        ...
        if algo_config["model"]["action_dim"] == 1:
            real_action_predict = [algo_config["model"]["action_config"]["acceleration"], action[0]]
...
def eval_new(env, agent,  env_config, algo_config, eval_config, mdl):
    ...
    for t_step in tqdm(range(max_timesteps)):
        ...
        state_innet = np.concatenate(((state_predict[:, 0] + state_predict[:, 1]) / 2,
                                     (state_predict[:, 2] + state_predict[:, 3]) / 2,
...
def eval_lka(config):
    #   创建日志，要求可重写
    logging.basicConfig(
        filename='./../../trained_lka_data.log',
        level=logging.INFO,
        filemode='w',
        format='%(asctime)s [%(levelname)s] %(message)s',
        handlers=[
            logging.FileHandler('./../../trained_lka_data.log'),  # 输出到文件
    ...
    logging.info('Episode y_dis v_y cos_h next_y_dis next_v_y next_cos_h')

...
def get_acc_states(csv_path, frequency, y_dis, v_y, cos_h, next_y_dis, next_v_y, next_cos_h, cost):
    state = utils_data.get_csv_info(csv_path, frequency, y_dis, v_y, cos_h)

    next_state = utils_data.get_csv_info(csv_path, frequency, next_y_dis, next_v_y, next_cos_h, cost)
    for item in state:
        item.append(0)

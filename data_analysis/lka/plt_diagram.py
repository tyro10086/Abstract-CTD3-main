...
from data_analysis.lka.abstracting_states import get_acc_states
from data_analysis.lka.mdp.construct_mdp import generate_graph_from_csv
from data_analysis.gap_statistic import compute_gap, gap_statistic
from eval_lka import eval_lka
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str, help="the path to save project",
                        default="project/20221231-TD3_risk-lanekeeping/checkpoint")
    ...
    parser.add_argument("--env_config", type=str, help="path of highway env",
                        default="conf/env/highway_kane_keeping_continuous_steering.yaml")
...
def plt_canopy(states, config):
    ...
    ax.scatter(states[:, 0], states[:, 1], states[:, 2], c=labels)
...
if __name__ == '__main__':
    ...
    log_file_train = '../../my_log_LKA.log'
    csv_file_train = './td3_risk_lka_logs.csv'

    #   训练好的模型里面的
    log_file_val = '../../trained_lka_data.log'
    csv_file_val = './td3_risk_lka_val_logs.csv'

    utils_data.log2csv(log_file_train, csv_file_train)

    #  从训练日志中读取数据
    train_data = utils_data.get_csv_info(csv_file_train, 1, 'y_dis', 'v_y', 'cos_h')
    ...
    # val_data = utils_data.get_csv_info(csv_file_val, 1, 'y_dis', 'v_y', 'cos_h',
    #                                    'y_dis_predict', 'v_y_predict', 'cos_h_predict')
    # val_data = np.array(val_data)
    #
    # y_true = val_data[:, 0:3]
    # y_predict = val_data[:, 3:6]
    #
